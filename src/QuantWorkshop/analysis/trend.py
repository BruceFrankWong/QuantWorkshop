...
def zigzag_hl(df: pd.DataFrame, echo: bool = False) -> tuple:
    df['zigzag_hl'] = 0
    pivot_point_list: list = []
    current_trend: Trend = Trend.Up             # 假设当前处于上升趋势。
    current_price: float = df.iloc[0, 'high']   # 假设当前目标价为下标0的最高价。
    df.iloc[0]['zigzag_hl'] = -1

    for index in range(1, len(df)):     # 从下标1开始循环。
        # 如果：当前趋势 == 上升趋势
        if current_trend == Trend.Up:

            # 如果： 当前下标的最高价 >= 当前目标价
            if df.iloc[index]['high'] >= current_price:
                status = '延续'
                current_price = df.iloc[index]['high']

            # 如果： 当前下标的最高价 < 当前目标价
            #    前一下标为转折点，前一下标的最高价为阶段最高价（极值）。
            else:
                status = '转折'
        ...
        # 如果：当前趋势 == 下降趋势
            ...
            if df.iloc[index]['low'] <= current_price:
                status = '延续'
                current_price = df.iloc[index]['low']
            else:
                status = '转折'
...
def zigzag_hl_without_review(df: pd.DataFrame, echo: bool = False) -> tuple:
    """
    用最高价和最低价做趋势判断。
...
def zigzag_hl2(df: pd.DataFrame, deviation: int = 5, retracement: int = 3, echo: bool = False) -> tuple:
    """
    用最高价和最低价做趋势判断。
    ...
    pivot_point_list: list = [(df.index[0], df.iloc[0]['low'])]     # 转折点列表
    high_list: list = []
    low_list: list = [(df.index[0], df.iloc[0]['low'])]
    cursor: float = df.iloc[0]['low']
    current_trend: Trend = Trend.Up
